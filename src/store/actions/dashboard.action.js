...
export const BlogList = (page) => {
  return async (dispatch) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const result = await axios.get(
        `${process.env.REACT_APP_ROOT}/api/blog/?page=${page}&limit=5`,
        {
          headers: {
            Authorization: token ? `Bearer ${token}` : "",
          },
...
export const DeleteBlogList = (blogId) => {
  return async (dispatch) => {
    dispatch({
      type: dashboardConstant.DELETE_EVENT_LIST_REQUEST,
    });
    try {
      const token = localStorage.getItem("adminToken");
      await axios.delete(`${process.env.REACT_APP_ROOT}/api/blog/${blogId}`, {
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
        },
      });
